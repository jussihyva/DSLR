...
/*   Updated: 2021/09/02 14:47:58 by jkauppi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	if (cmd_param_type == E_OPTIONAL_SHORT)
	{
		while (*(++arg))
		{
			if (pre_analyse_argument(arg_parser->options, *arg, argc_argv))
				fn_input_param_save(arg_parser->input_params, *arg, argc_argv,
					cmd_param_type);
			else
			{
...
void	ft_arg_parser(t_arg_parser *arg_parser)
{
	...
	arg_parser->input_params
		= (void *)arg_parser->fn_input_params_initialize(argc_argv);
